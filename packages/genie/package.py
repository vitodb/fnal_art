...
"""
    ...
    resource(url='https://github.com/GENIE-MC/Reweight/archive/R-1_00_06.tar.gz', 
             when="@3.00.00:", destination="reweight.tar.gz")
    ...
    def add_to_configure_env(self):
        tar = which("tar")
        tar("xzvf", "reweight.tar.gz")
        dname = glob("%s/Reweight-*" % self.stage.source_path)
        os.rename(dname, "%s/Reweight" % self.stage.source_path)
        inspect.getmodule(self).configure.add_default_env('GENIE',self.stage.source_path)
        inspect.getmodule(self).configure.add_default_env('GENIE_REWEIGHT',"%s/Reweight" % self.stage.source_path)

    @run_before('build')
    def add_to_make_env(self):
        inspect.getmodule(self).make.add_default_env('GENIE',self.stage.source_path)
        inspect.getmodule(self).make.add_default_env('GENIE_REWEIGHT',"%s/Reweight" % self.stage.source_path)
        inspect.getmodule(self).make.add_default_env('LD_LIBRARY_PATH', '{0}/lib'.format(self.stage.source_path))

    @run_after('build')
    def build_reweight(self):
        os.mkdir("%s/Reweight" % self.build_directory)
        with working_dir("%s/Reweight" % self.build_directory):
            make("-f","%s/Reweight/Makefile" % self.stage.source_path, "all")

    @run_after('install')
    def install_reweight(self):
        with working_dir("%s/Reweight" % self.build_directory):
            make("-f","%s/Reweight/Makefile" % self.stage.source_path, "install")
    @run_after('install')
